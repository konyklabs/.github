...
Receipts are not a new datastore. Every run sets `run-name` to
`role/<role>/<job>`, so the Actions run history *is* the receipt, and
`bin/heartbeat.sh` reads it back with `gh run list`. A job that stops firing
