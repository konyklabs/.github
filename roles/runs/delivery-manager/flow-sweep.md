...
2. Across every repo in the org — `gh api /orgs/konyklabs/repos --jq '.[].name'` — collect:
   - open PRs: `gh pr list --repo konyklabs/<r> --state open --json number,title,createdAt,updatedAt,isDraft,statusCheckRollup`
   - branches: `gh api /repos/konyklabs/<r>/branches --jq '.[].name'`
   - recent runs: `gh run list --repo konyklabs/<r> --limit 20`
3. Compute, do not estimate:
...
4. Before proposing a comment, check the issue's recent comments for the `konyklabs-role` footer. If a previous sweep already said this, say nothing.
